...
/// Purpose: Trainer interaction event for Pokémon Unity frontend
...
public class InteractTrainer : MonoBehaviour
{
    ...
    {
        float increment = 0f;
        float speed = Player.player.walkSpeed;
    ...
    public IEnumerator spotPlayer()
    {
        if (!defeated && !busy)
        {
            ...
            if (Player.player.setCheckBusyWith(this.gameObject))
            {
                ...
                Player.player.direction = (CharacterBase.Direction)flippedDirection;
