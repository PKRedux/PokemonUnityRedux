...
/// Purpose: Saving and loading serializable variables for Pokémon Unity backend
...
namespace PokemonUnity.Backend.FileIO {
public static class SaveLoad
{
    ...
    public static void Save()
    {
        if (SaveData.currentSave != null)
        {
            if (SaveData.currentSave.getFileIndex() >= 0 && SaveData.currentSave.getFileIndex() < savedGames.Length)
            {
                ...
                FileStream file = File.Create(Application.persistentDataPath + "/playerData.pkunity");
    ...
    public static bool Load()
    {
        ...
        if (File.Exists(Application.persistentDataPath + "/playerData.pkunity"))
        {
            BinaryFormatter bf = new BinaryFormatter();
            FileStream file = File.Open(Application.persistentDataPath + "/playerData.pkunity", FileMode.Open);
